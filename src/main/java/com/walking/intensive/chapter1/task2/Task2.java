...
//        Для собственных проверок можете делать любые изменения в этом методе
    ...
    static String getFlatLocation(int floorAmount, int entranceAmount, int flatNumber) {
        ...
        if (flatNumber > wholeHouseCapacity) {
            ...
            switch (flatLocation) {
                case (0):
                    flatCalcResult = flatNumber + " кв - " + exactEntranceCalc + " подъезд, " + exactFloorCount + " этаж, " + flatOptions[3];
                    break;
                case (1):
                    flatCalcResult = flatNumber + " кв - " + exactEntranceCalc + " подъезд, " + exactFloorCount + " этаж, " + flatOptions[0];
                    break;
                case (2):
                    flatCalcResult = flatNumber + " кв - " + exactEntranceCalc + " подъезд, " + exactFloorCount + " этаж, " + flatOptions[1];
                    break;
                case (3):
                    flatCalcResult = flatNumber + " кв - " + exactEntranceCalc + " подъезд, " + exactFloorCount + " этаж, " + flatOptions[2];
                    break;
            }
